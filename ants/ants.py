        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if not (isinstance(self.ant.ant, QueenAnt) and not self.ant.ant.impostor):
                        self.ant.ant = None
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        location_bee = self.place
        while location_bee is not hive:
            if len(location_bee.bees) == 0:
                location_bee = location_bee.entrance
            else:
                return random_or_none(location_bee.bees)
        # END Problem 3B
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        tran = 0
        tunnel = self.place
        plbee = self.place.bees
        while tran < self.min_range:
            tunnel = tunnel.entrance
            tran += 1
        while tunnel.bees == [] and type(tunnel) == Place:
            tunnel = tunnel.entrance
        if tunnel != hive and type(tunnel) == Place:
            findbees = tunnel.bees
        else:
            findbees = plbee
        return random_or_none(findbees)  
    # END Problem 4B
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        tunnel = self.place
        plbee = self.place.bees
        while (tunnel.bees == [] and type(tunnel) == Place) and tran < self.max_range:
            tunnel = tunnel.entrance
            tran += 1
        if tunnel != hive and type(tunnel) == Place:
            findbees = tunnel.bees
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        tanked_bees = self.place.bees[:]
        for bee in tanked_bees:
            bee.reduce_armor(self.damage)
            # if len(self.place.bees) == 1:
            #     random_or_none(self.place.bees).reduce_armor(self.damage)
        # END Problem 8
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        # if self.container:
        #     ant_contained = penetration.ant 
        #     if ant_contained and not ant_contained in self.ants_doubled:
        #         ant_contained.damage *= 2
        #         self.ants_doubled.append(ant_contained)
        else:
            ThrowerAnt.action(self, colony)
            penetration = self.place.exit
            while not (penetration is None):
                if not (penetration.ant is None):
                    if not (penetration.ant in QueenAnt.ants_doubled):
                        penetration.ant.damage *= 2
                        QueenAnt.ants_doubled.append(penetration.ant)
                    if penetration.ant.container:
                        # if not (penetration.ant.container) is None:
                        ant_contained = penetration.ant.ant
                        if ant_contained and not ant_contained in self.ants_doubled:
                            ant_contained.damage *= 2
                            self.ants_doubled.append(ant_contained)
                penetration = penetration.exit
        # END Problem 9
